...
df1=pd.read_csv('./data/US_Accidents_Dec19.csv')
...
df['Severity']=df['Severity'].apply(lambda x: 'Low' if x<3 else 'High')
print(df['Severity'].head(3))

print(df.head(5))
print(df[df['Severity']=='High'].count)
print(df[df['Severity']=='Low'].count)

#Plot Severity
labels= df['Severity'].unique()
sizes= df.groupby("Severity")["Severity"].count()
explode = (0, 0)
...
print(data)
dataFrame = pd.DataFrame(data=data, index=sizes)
print(dataFrame)
print('######################################')
